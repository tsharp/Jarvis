...
export function initToolsApp() {
    ...
    if (window.TRIONBridge) {
        // MCP list now loaded directly via API in refreshTools()
        window.TRIONBridge.on('mcp:install_success', (data) => {
            showLog(`✅ Success: ${data.package}`, 'text-green-400');
            setInstalling(false);
            refreshTools();
        });
        window.TRIONBridge.on('mcp:install_error', (data) => {
            showLog(`❌ Error: ${data.error}`, 'text-red-400');
            setInstalling(false);
        });
    }
}
...
function bindEvents() {
    ...
    installBtn?.addEventListener("click", () => {
        ...
        setInstalling(true);
        showLog(`📦 Installing ${pkg}... (this may take a moment)`, 'text-gray-300');
    ...
    if (uploadBtn && zipInput && statusEl) {
        uploadBtn.addEventListener("click", async function() {
            if (!zipInput.files || zipInput.files.length === 0) {
                alert("Bitte eine ZIP-Datei auswaehlen!");
                return;
            }
            ...
            try {
                ...
                statusEl.textContent = "Success: " + data.name + " installed!";
...
async function refreshTools() {
    ...
    tbody.innerHTML = `<tr><td colspan="3" class="p-8 text-center text-gray-500">
        <i data-lucide="loader-2" class="w-6 h-6 animate-spin mx-auto mb-2"></i>
        Loading MCPs...
    </td></tr>`;
    if (window.lucide) window.lucide.createIcons();
...
function renderToolsList(mcps) {
    const tbody = document.getElementById("tools-list-body");
    if (!tbody) return;

    if (!mcps || mcps.length === 0) {
        tbody.innerHTML = `<tr><td colspan="3" class="p-8 text-center text-gray-500">No MCPs installed</td></tr>`;
        return;
    }

    const rows = mcps.map(mcp => {
        const statusColor = mcp.enabled ? 'green' : 'gray';

...
try:
    from cim_rag import cim_kb  # type: ignore
except Exception:
    class _FallbackCimKB:
        """Cross-service fallback when CIM-RAG is not available."""
        def load(self):
            return None

        def get_template_by_intent(self, _intent: str):
            return None

        def get_persona(self, _name: str):
            return ""

    cim_kb = _FallbackCimKB()
...
# Mapping: import module -> pip/distribution package name.
# This avoids false "missing package" detections for modules whose
# import name differs from installation package.
_IMPORT_TO_PACKAGE = {
    "bs4": "beautifulsoup4",
    "pil": "pillow",
...
class AutonomousTaskResult:
    ...
    def to_dict(self) -> Dict:
        ...
        # C7: pending_package_approval — full signal for UI/Caller and approval tracking
        elif self.action_taken == "pending_package_approval":
            result["needs_package_install"] = True
            result["needs_package_approval"] = True
            result["event_type"] = "approval_requested"
            if self.error and self.error.startswith("missing_packages:"):
                pkgs = self.error.split(":", 1)[1].split(",")
                result["missing_packages"] = [p.strip() for p in pkgs if p.strip()]
        return result
...
class SkillMiniControl:
    ...
    def _is_skill_discovery_enabled() -> bool:
        """C10 (Rest) rollout/rollback gate for autonomous discovery."""
        try:
    ...
    async def process_autonomous_task(
        ...
        prefer_create = bool(getattr(task, "prefer_create", False)) or self._is_explicit_create_request(
            task.intent, task.user_text
        )
        ...
        # 3.5. PACKAGE CHECK + C7 Policy — vor Validation, nach Code-Generierung
        try:
            from config import get_skill_package_install_mode as _gpim
            _pkg_mode = _gpim()
        except Exception:
        ...
        _cd = {
            "action": "warn" if _has_medium else "approve",
            "passed": True,
            "reason": "Autonomous validation passed",
            "warnings": [
                i.description for i in validation.issues if i.severity.lower() == "medium"
            ] + scanner_warnings,
            "validation_score": validation.score,
            "source": "skill_server",
            "policy_version": "1.0",
    ...
    async def _find_matching_skill(
        ...
        """
        # Load installed skills (truth source)
        ...
        # C10 (Rest): read-only autonomous discovery with truth-filter to prevent drift.
        # Graph candidates are used only as hints; any candidate not in truth map is ignored.
        if self._is_skill_discovery_enabled():
            graph_match = await self._discover_matching_skill_via_graph(
                intent=intent,
                user_text=user_text,
                truth_skills=all_skills,
            )
            if graph_match:
                return graph_match
    ...
    @staticmethod
    def _is_explicit_create_request(intent: str, user_text: str) -> bool:
        combined = f"{intent} {user_text}".lower()
        create_markers = (
            "erstelle",
            "erstellen",
            "create",
        ...
        return any(m in combined for m in create_markers) and not any(
            m in combined for m in existing_markers
        )

    async def _discover_matching_skill_via_graph(
        self,
        *,
        intent: str,
        user_text: str,
        truth_skills: Dict[str, Dict],
    ) -> Optional[Dict[str, Any]]:
        """
        Read-only graph discovery with strict truth cross-check.
        Prevents ghost-skill drift by accepting only candidates present in truth_skills.
        """
        rows = await self._fetch_skill_graph_candidates(
            query=f"{intent} {user_text}".strip(),
            limit=8,
        )
        if not rows:
            return None

        # canonical name map for strict truth cross-check
        truth_by_norm = {str(name).strip().lower(): (name, info) for name, info in truth_skills.items()}

        drift_hits = 0
        for row in rows:
            candidate_name = self._extract_graph_skill_name(row)
            if not candidate_name:
                continue
            norm = candidate_name.strip().lower()
            if norm not in truth_by_norm:
                drift_hits += 1
                continue
            real_name, real_info = truth_by_norm[norm]
            return {
                "name": real_name,
                **real_info,
                "match_score": float(row.get("similarity", 0.0) or row.get("score", 0.0) or 0.0),
                "discovery_source": "graph_truth_filtered",
            }

        if drift_hits:
            print(f"[MiniControl] Discovery drift filtered: {drift_hits} ghost candidates ignored")
        return None

    async def _fetch_skill_graph_candidates(self, query: str, limit: int = 8) -> List[Dict[str, Any]]:
        """
        Query graph index candidates from sql-memory.
        Read-only helper; failures return [] for safe fallback to local matching.
        """
        payload = {
            "jsonrpc": "2.0",
    ...
    @staticmethod
    def _extract_graph_skill_name(row: Dict[str, Any]) -> str:
        """
        Extract skill name from graph row metadata/content.
        Supports both metadata dict and metadata JSON string shapes.
        """
        ...
        name = str(meta.get("skill_name") or meta.get("name") or "").strip()
        if name:
            return name
    ...
    def _load_installed_skills(self) -> Dict[str, Dict]:
        ...
        Handles both legacy flat dict and V2 envelope transparently.
        Returns flat {skill_name: {...}} — backward compat for all call sites.
        """
        ...
        if registry_file.exists():
            try:
                ...
                if isinstance(raw, dict) and raw.get("schema_version") == 2:
                    return raw.get("skills", {}) if isinstance(raw.get("skills"), dict) else {}
    ...
    def _load_draft_skills(self) -> Dict[str, Dict]:
        """Load draft skills (for matching). Supports manifest.yaml and legacy manifest.json."""
...
"""
    ...
    def _module_to_package_name(module_name: str) -> str:
        """Normalize import module name to pip/distribution package name."""
    ...
    @staticmethod
    def _parse_packages(pkgs) -> set:
        """
        Parse both package list shapes into a lowercase name set.
          - list[str]            → from /v1/packages/installed (compat shape)
          - list[dict{name,...}] → from /v1/packages (UI shape)
        """
        result = set()
        for p in pkgs:
            if isinstance(p, str):
    ...
    async def _get_installed_packages(self) -> set:
        ...
        Mode via EXECUTOR_PACKAGES_ENDPOINT_MODE (default: auto):
          auto   → /v1/packages/installed first, then /v1/packages fallback
          compat → /v1/packages/installed only  (no fallback endpoint)
          modern → /v1/packages only            (dict-list shape)
        Fail-closed final fallback: _KNOWN_INSTALLED.

        Rollback: set EXECUTOR_PACKAGES_ENDPOINT_MODE=modern to skip the
        compat endpoint entirely and use /v1/packages directly.
        """
        executor_url = os.getenv("EXECUTOR_URL", "http://tool-executor:8000")
        mode = os.getenv("EXECUTOR_PACKAGES_ENDPOINT_MODE", "auto").lower()
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                # ── compat or auto: try /v1/packages/installed first ──────
                if mode in ("auto", "compat"):
                    r = await client.get(f"{executor_url}/v1/packages/installed")
                    if r.status_code == 200:
                        return self._parse_packages(r.json().get("packages", []))
                    # compat-only: no further fallback endpoint
                    if mode == "compat":
                        return set(_KNOWN_INSTALLED)

                # ── modern or auto-fallback: use /v1/packages ─────────────
                r = await client.get(f"{executor_url}/v1/packages")
                if r.status_code == 200:
                    return self._parse_packages(r.json().get("packages", []))
        except Exception:
            pass
    ...
    async def _check_missing_packages(self, code: str) -> List[str]:
        ...
        Gibt Liste der fehlenden Drittanbieter-Pakete (pip-Namen) zurück.
        """
        all_imports = self._extract_package_names(code)
        # stdlib rausfiltern — immer verfügbar
        third_party_modules = [p for p in all_imports if p not in _STDLIB_MODULES]
        if not third_party_modules:
            return []

        # Convert module imports to pip package names for allowlist/install parity.
        third_party_packages: List[str] = []
        for module_name in third_party_modules:
            pkg_name = self._module_to_package_name(module_name)
            if pkg_name and pkg_name not in third_party_packages:
                third_party_packages.append(pkg_name)

        installed = await self._get_installed_packages()
        return [p for p in third_party_packages if p.lower() not in installed]

    async def _get_package_allowlist(self) -> set:
        """
        C7: Fetch the package allowlist from tool-executor GET /v1/packages.

        Returns a set of lowercase allowlisted package names.
        Fail-closed: returns empty set on any error (all packages treated as
        non-allowlisted → pending_package_approval on error).
        """
        executor_url = os.getenv("EXECUTOR_URL", "http://tool-executor:8000")
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if r.status_code == 200:
                    data = r.json()
                    return {pkg.lower() for pkg in data.get("allowlist", [])}
        except Exception:
            pass
        return set()  # fail-closed: empty allowlist = nothing auto-approved

    async def _auto_install_packages(self, packages: List[str]) -> Dict[str, Any]:
        """
        C7: Install allowlisted packages via POST /v1/packages/install.

        Returns {"success": True} on full success.
        Returns {"success": False, "error": str} on first failure (fail-closed).
        """
        executor_url = os.getenv("EXECUTOR_URL", "http://tool-executor:8000")
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                for pkg in packages:
                    r = await client.post(
                        f"{executor_url}/v1/packages/install",
                        json={"package": pkg},
                    )
                    data = r.json()
                    if not data.get("success"):
                        return {
                            ...
                            "error": data.get(
                                "error",
                                f"Package '{pkg}' installation failed",
                            ),
                        }
            return {"success": True}
        except Exception as e:
    ...
    async def _install_skill(
        ...
        C4.5: control_decision (already made by this service) is forwarded so

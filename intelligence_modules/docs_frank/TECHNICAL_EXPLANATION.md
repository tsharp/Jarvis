...
### 2. Specialized Node Types (Influence Diagrams)
Beyond simple variables, the module utilizes specialized nodes to handle decision-making and logic:
*   **Decision Nodes (Rectangles):** Represent interventions or choices (e.g., "Change Price").
*   **Utility Nodes (Diamonds):** Represent the objective function or goal (e.g., "Maximize LTV").
*   **Logic Gates (Triangles):** Active fallacy-mitigation points (e.g., "Check for Simpson's Paradox").
*   **Counterfactual Nodes:** Parallel world-state variables for scenario branching.

### 3. The do-calculus Implementation

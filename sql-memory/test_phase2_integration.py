...
def cleanup_test_data():
    try:
        ...
        try:
            ...
            conn.execute("DELETE FROM embeddings WHERE content_type = 'task' AND content LIKE ?", (f'%{TEST_CONVERSATION_ID}%',))
            conn.commit()
...
def test_1_flush_mechanism():
    ...
    try:
        ...
        for i in range(15):
            task_data = {"request_id": f"test_req_{i}", "conversation_id": TEST_CONVERSATION_ID, "intent": f"Test task {i}", "status": "running"}
        ...
        conn = sqlite3.connect(str(DB_PATH))
        try:
            cursor = conn.execute("SELECT COUNT(*) FROM task_active WHERE conversation_id = ?", (TEST_CONVERSATION_ID,))
            active_count = cursor.fetchone()[0]
            cursor = conn.execute("SELECT COUNT(*) FROM task_archive WHERE conversation_id = ?", (TEST_CONVERSATION_ID,))
            ...
            if archive_count == 5:
                ...
                return False
            cursor = conn.execute("SELECT task_data FROM task_archive WHERE conversation_id = ? ORDER BY created_at", (TEST_CONVERSATION_ID,))
...
def test_2_auto_expiry():
    ...
    try:
        ...
        try:
            ...
            for i in range(3):
                conn.execute("INSERT INTO task_active (request_id, conversation_id, task_data, created_at, last_updated, importance_score) VALUES (?, ?, ?, ?, ?, ?)", (f"old_req_{i}", TEST_CONVERSATION_ID, f'{{"intent": "Old task {i}", "status": "completed"}}', old_timestamp, old_timestamp, 0.5))
            print_info("Inserting 2 fresh tasks...")
            for i in range(2):
                conn.execute("INSERT INTO task_active (request_id, conversation_id, task_data, created_at, last_updated, importance_score) VALUES (?, ?, ?, ?, ?, ?)", (f"new_req_{i}", TEST_CONVERSATION_ID, f'{{"intent": "New task {i}", "status": "running"}}', now.isoformat(), now.isoformat(), 0.5))
            conn.commit()
        finally:
            conn.close()
        from core.lifecycle.task import TaskLifecycleManager
...
def test_3_embedding_pipeline():
    ...
    try:
        ...
        conn = sqlite3.connect(str(DB_PATH))
        try:
            test_task_data = {"intent": "Find information about Python decorators", "status": "completed", "result": "Found 5 relevant articles about Python decorators"}
            print_info("Inserting task into archive (without embedding)...")
            conn.execute("INSERT INTO task_archive (request_id, conversation_id, task_data, created_at, archived_at, embedding_id) VALUES (?, ?, ?, ?, ?, NULL)", ("embed_test_req", TEST_CONVERSATION_ID, str(test_task_data), datetime.now().isoformat(), datetime.now().isoformat()))
            conn.commit()
        finally:
            conn.close()
        ...
        conn = sqlite3.connect(str(DB_PATH))
        try:
            cursor = conn.execute("SELECT embedding_id FROM task_archive WHERE conversation_id = ?", (TEST_CONVERSATION_ID,))
            row = cursor.fetchone()
            if row and row[0] is not None:
                embedding_id = row[0]
                print_pass(f"Embedding created (ID: {embedding_id})")
                cursor = conn.execute("SELECT content_type FROM embeddings WHERE id = ?", (embedding_id,))
...
def main():
    ...
    print(f"{Colors.BLUE}Running inside Docker container{Colors.RESET}")
    print(f"{Colors.BLUE}{'='*60}{Colors.RESET}")
    print_info(f"Test Conversation ID: {TEST_CONVERSATION_ID}")
    print_info(f"Database: {DB_PATH}")
    results = {"Test 1: Flush-Mechanismus": False, "Test 2: 48h Auto-Expiry": False, "Test 3: Embedding-Pipeline": False}
    try:
